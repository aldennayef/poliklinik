                        ...
                        <form class="form-horizontal" id="periksaPasienForm">
                            <div class="card-body">
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="datetime-local" class="form-control" id="tanggal" name="tanggal" value="<?=$pasien['tgl_periksa']?>"
                                        placeholder="Masukkan Jam Mulai">
                                </div>
                                <div class="form-group">
                                    <label for="catatan">Catatan Periksa</label>
                                    <textarea class="form-control" name="catatan"><?=$pasien['catatan']?></textarea>
                                </div>
                                <div id="obat-container">
                                    <?php foreach ($detail_periksa as $dp): ?>
                                    <div class="input-group mt-2">
                                        <select class="form-control obat" name="obat[]">
                                            <option value="">Pilih Obat</option>
                                            <?php foreach($obat as $listobat): ?>
                                            <option value="<?=$listobat['harga']?>" data-id="<?=$listobat['id']?>"
                                                <?php if($dp['id_obat'] == $listobat['id']) echo 'selected'; ?>>
                                                <?=$listobat['nama_obat']?> - <?=$listobat['kemasan']?> -
                                                <?=$listobat['harga']?>
                                            </option>
                                            <?php endforeach; ?>
                                        </select>
                            ...
                            <div class="card-footer">
                                <button type="submit" class="btn btn-info btnDokterSubmitEditPeriksaPasien">Submit</button>
                            </div>
                            <!-- /.card-footer -->
                        </form>
                    </div>
